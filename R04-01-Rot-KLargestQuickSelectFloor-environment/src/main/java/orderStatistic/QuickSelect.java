...
public class QuickSelect<T extends Comparable<T>> {
	...
	public T quickSelect(T[] array, int k) {
		...
		if(array.length > 0 && k > 0 && k <= array.length) {
			res = recursiveQuickSelect(array,k,0,array.length - 1);
		}
	...
	private int partition(T [] array, int low, int high) {
		...
		media3(array,low ,middle,high );
		T pivot = array[middle];
		Util.swap(array,middle,high-1);
		int pivotIndex = high - 1;

		for(int j = pivotIndex-1; j > low;j-- ) {
			if(pivot.compareTo(array[j]) < 0) {
				pivotIndex--;
				Util.swap(array,pivotIndex,j);
			}
		}
		Util.swap(array,high-1,pivotIndex);

		return pivotIndex;
	}

	private void media3(T[] array, int leftIndex, int middleIndex, int rightIndex) {
